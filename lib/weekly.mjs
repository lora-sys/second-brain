...
// v0.18 — pickProvider accepts LLM opts (ADR-0008). The require()-based
// path that lived here before was broken in ESM (require is not defined
// in .mjs files), so weekly.mjs silently fell back to local-echo the
// whole time. Now uses proper ESM imports.
export function pickProvider(opts = {}) {
  ...
  if (apiKey || baseURL) {
    try {
      ...
      console.warn('[weekly] failed to create OpenAI provider, falling back to local-echo:', err.message);
    }
  }
...
export async function generateWeekly({ date, eventsByDay, staleTasks = [], provider = null, vaultPath = null, llm = null }) {
