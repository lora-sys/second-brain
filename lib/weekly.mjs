...
function localEchoWeekly(dateStr, summary, staleTasks) {
...
export async function generateWeekly({ date, eventsByDay, staleTasks = [], provider = null }) {
  ...
  if (usedProvider.info().name === 'local-echo') {
    body = localEchoWeekly(dateStr, summary, staleTasks);
  } else {
    // For real LLM, send a richer prompt (not implemented for v0.7 stub)
    body = localEchoWeekly(dateStr, summary, staleTasks);
  }
  const fm = [
    '---',
