...
async function route(req, res) {
  ...
  const method = req.method;
    ...
    // v0.30 — Cockpit Agent: real LLM call. Body: { prompt: string, maxTokens?, temperature? }.
    //   Reads cfg.llm to choose provider, builds a compact system prompt from
    //   the vault, calls the configured provider, returns the text + provider info.
    //   On error, returns 200 with error set so the client's local-echo fallback
    //   can take over without the page crashing.
    if (pathname === '/api/agent' && method === 'POST') {
      try {
        const body = await readBody(req).catch(() => ({}));
        const prompt = (body && body.prompt) || '';
        if (!prompt.trim()) return jsonResponse(res, 400, { ok: false, error: 'prompt is required' });
        const cfg = await getConfig();
        const llmOpts = getLlmOpts(cfg);
        // runAgent only needs the *path string*, not the Vault object.
        // An empty vaultPath yields an empty system-prompt summary (fine).
        const result = await runAgent({
          prompt,
          vaultPath: cfg.vaultPath || null,
          llm: llmOpts,
          maxTokens: Number(body.maxTokens) > 0 ? Number(body.maxTokens) : 500,
          temperature: body.temperature != null ? Number(body.temperature) : 0.4,
        });
