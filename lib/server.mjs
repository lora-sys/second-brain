...
async function handleWeeklyGenerate(req, res, events, vault) {
  ...
  try {
    const { content, provider } = await generateWeekly({ date: dateStr, eventsByDay, staleTasks, vaultPath: cfg.vaultPath });
...
async function handleDailyGenerate(req, res, events, vault) {
  ...
  try {
    const { content, provider } = await generateDaily({ date: dateStr, eventsByDay, recentJournals });
