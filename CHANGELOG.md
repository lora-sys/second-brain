...
- **v0.18 — In-app LLM configuration panel (ADR-0008)** — settings page now exposes `API Key`, `Base URL`, `Model` with redaction on `GET /api/config` (mask as `••••••••<last4>` + `configured` flag). New `POST /api/llm/test` pings the configured provider. `pickProvider` in `lib/{daily,weekly}.mjs` accepts an `opts` block plumbed from server.mjs's `getLlmOpts`. **Bug fix**: weekly.mjs's `pickProvider` was using `require('./llm/openai.mjs')` in an ESM file — silently fell back to local-echo. Now a proper ESM import. 33 new unit tests in `tests/llm-config.test.mjs`. 4 new E2E tests for redaction + mask-as-no-op semantics. No new npm deps.
